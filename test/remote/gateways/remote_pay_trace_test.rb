...
class RemotePayTraceTest < Test::Unit::TestCase
  ...
  def test_token_validity
    response = @gateway.check_token_response
  end
  ...
  #   response = @gateway.purchase(29, @mastercard, @options)
  ...
  def test_successful_refund
    # This test may prove to be difficult as you can only refund a settled transaction and there is a lag time of unknown amount for it to be settled
    purchase = @gateway.purchase(@amount, @mastercard, @options)
  ...
  def test_invalid_login
    gateway = PayTraceGateway.new(login: '', password: '')
